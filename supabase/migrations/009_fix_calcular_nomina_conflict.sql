-- Migration: Fix calcular_nomina to use ON CONFLICT for true UPSERT behavior
-- This requires the unique constraint 'unique_payroll_period' to be present (created in 008)
...
BEGIN
    ...
    LOOP
        ...
        -- We now rely on the unique constraint (user_id, fecha_inicio_periodo)
        ...
        -- Ensure all events in this range are linked to the upserted period
