...
BEGIN
    ...
    LOOP
        ...
        -- FALTAS: Descuentan un día completo SOLO si no están justificadas
        ...
        WHERE user_id = v_user_record.user_id 
          ...
          AND (justificado IS FALSE OR justificado IS NULL);

        -- TARDANZAS: Si no tienen monto, calcular por horas o poner 0.5h (medio turno de tardanza sugerido)
        ...
        -- APERCIBIMIENTOS: Según convención, descuentan medio día si no tienen monto
        UPDATE eventos_nomina SET monto = v_valor_dia / 2.0
        WHERE user_id = v_user_record.user_id 
          AND fecha_evento >= p_fecha_inicio AND fecha_evento <= p_fecha_fin
          AND tipo = 'APERCIBIMIENTO' AND (monto < 0.01 OR monto IS NULL)
          AND (justificado IS FALSE OR justificado IS NULL);

        -- ASEGURAR que todo lo JUSTIFICADO tenga monto 0
        UPDATE eventos_nomina SET monto = 0 
        WHERE user_id = v_user_record.user_id 
          AND fecha_evento >= p_fecha_inicio AND fecha_evento <= p_fecha_fin
          AND justificado IS TRUE;

        -- HORAS EXTRAS: Siempre calcular si no tienen monto
        ...
        IF v_period_id IS NOT NULL THEN
            ...
            WHERE id = v_period_id AND estado = 'CALCULADO';
        ELSE
